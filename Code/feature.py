...
class Feature(object):
    ...
    def generate_initial_guess(self, T_c1_c2, z1, z2):
        ...
        m = T_c1_c2.R @ np.array([*z1, 1.0])  # Backproject depth from 
        a = m[:2] - z2*m[2]                   # vec2
        b = z2*T_c1_c2.t[2] - T_c1_c2.t[:2]   # vec2 -> Relates translation between two frames. If this is zero, depth becomes zero.
    ...
    def initialize_position(self, cam_states):
        ...
        for pose in cam_poses:
            ...
            if position[2] <= 0 or np.isnan(position).any() or np.any(np.isinf(position)):
