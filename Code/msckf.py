...
class MSCKF(object):
    ...
    def initialize_gravity_and_bias(self):
        ...
        for imu_msg in self.imu_msg_buffer:
            angular_vel=np.array(imu_msg.angular_velocity)
            linear_acc=np.array(imu_msg.linear_acceleration)
        ...
        gravity_mag=np.linalg.norm(gravity_imu)

        IMUState.gravity =np.array([0,0,-gravity_mag])

        q_imu_world=from_two_vectors(-IMUState.gravity, gravity_imu)

        # Initialize the initial orientation, so that the estimation
        # is consistent with the inertial frame.
        self.state_server.imu_state.orientation=q_imu_world
    ...
    def batch_imu_processing(self, time_bound):
        ...
        # Counter for how many IMU messages were used
        imu_msg_counter = 0

        for imu_msg in self.imu_msg_buffer:
            imu_time = imu_msg.timestamp #/1e9

            if imu_time < (self.state_server.imu_state.timestamp): #/1e9
                imu_msg_counter += 1
                continue

            if imu_time > time_bound: #/1e9:
                break

            # Convert IMU msg fields to NumPy arrays
            m_gyro = np.array(imu_msg.angular_velocity)
            m_acc = np.array(imu_msg.linear_acceleration)
    ...
    def process_model(self, time, m_gyro, m_acc):
        ...
        dt = (time - imu_state.timestamp) #/1e9
        ...
        F[0:3, 0:3] = -skew(gyro)
        F[0:3, 3:6] = -np.eye(3)
        F[6:9, 0:3] = -to_rotation(imu_state.orientation).T @ skew(acc)
        ...
        # Pseudo-inverse-like projection: s = (u^T * u)^-1 * u^T
        # This results in a 1x3 row vector
        dot = u.T @ u
        s = (1.0 / dot) * u.T
        ...
        # Check if any camera states are currently part of the state vector (i.e. state augmentation has occurred, like in MSCKF).
        if len(self.state_server.cam_states) > 0:
            imu_size = 21

            # IMU-to-camera cross-covariance
            self.state_server.state_cov[:imu_size, imu_size:] = (
                Phi @ self.state_server.state_cov[:imu_size, imu_size:]
            )

            # Camera-to-IMU cross-covariance
            self.state_server.state_cov[imu_size:, :imu_size] = (
                self.state_server.state_cov[imu_size:, :imu_size] @ Phi.T
            )

        # Fix the covariance to be symmetric
        state_cov_fixed = (self.state_server.state_cov + self.state_server.state_cov.transpose()) / 2.0
    ...
    def predict_new_state(self, dt, gyro, acc):
        ...
        if gyro_norm > 1e-5:
            dq_dt = (np.cos(0.5 * gyro_norm * dt) * np.eye(4) +
                    (np.sin(0.5 * gyro_norm * dt) / gyro_norm) * Omega) @ q
            dq_dt2 = (np.cos(0.25 * gyro_norm * dt) * np.eye(4) +
                    (np.sin(0.25 * gyro_norm * dt) / gyro_norm) * Omega) @ q
        else:
            dq_dt = ((np.eye(4) + 0.5 * dt * Omega) *
                    np.cos(0.5 * gyro_norm * dt)) @ q
            dq_dt2 = ((np.eye(4) + 0.25 * dt * Omega) *
                    np.cos(0.25 * gyro_norm * dt)) @ q
        
        # Convert quaternion [x, y, z, w] → rotation matrix → transpose
        dR_dt_transpose = to_rotation(dq_dt).T
        dR_dt2_transpose = to_rotation(dq_dt2).T

        # Apply 4th order Runge-Kutta 

        gravity = self.state_server.imu_state.gravity

        # k1 = f(tn, yn)
        k1_v_dot = to_rotation(q).T @ acc + gravity
        k1_p_dot = v

        # k2 = f(tn+dt/2, yn+k1*dt/2)
        k1_v = v + 0.5 * dt * k1_v_dot
        k2_v_dot = dR_dt2_transpose @ acc + gravity
        k2_p_dot = k1_v
                
        # k3 = f(tn+dt/2, yn+k2*dt/2)
        k2_v = v + 0.5 * dt * k2_v_dot
        k3_v_dot = dR_dt2_transpose @ acc + gravity
        k3_p_dot = k2_v
        
        # k4 = f(tn+dt, yn+k3*dt)
        k3_v = v + dt * k3_v_dot
        k4_v_dot = dR_dt_transpose @ acc + gravity
        k4_p_dot = k3_v


        # yn+1 = yn + dt/6*(k1+2*k2+2*k3+k4)
        q = quaternion_normalize(dq_dt)
    ...
    def state_augmentation(self, time):
        ...
        """
        R_i_c = self.state_server.imu_state.R_imu_cam0    #####it means cam0 w.r.t to imu
        t_c_i = self.state_server.imu_state.t_cam0_imu      ####translation of imu w.r.t camera

        # Compute camera pose in world frame
        R_w_i = to_rotation(self.state_server.imu_state.orientation)
        R_w_c = R_i_c @ R_w_i
        t_c_w = self.state_server.imu_state.position + R_w_i.T @ t_c_i ## as we are taking translation imu w.r.t cam that is why taken full

        # Create a new CAMState
        cam_state = CAMState(self.state_server.imu_state.id) ###maps its id to imu --so that later we know which group is it of imu and camera
        cam_state.time = time
        cam_state.orientation = to_quaternion(R_w_c)  ####conversion of above derived rotation vector
        cam_state.position = t_c_w
        cam_state.orientation_null = cam_state.orientation
        cam_state.position_null = cam_state.position

        self.state_server.cam_states[self.state_server.imu_state.id] = cam_state  #####hash maps just the address to corresponding id(itself)

        # Construct the Jacobian J
        J = np.zeros((6, 21))
        J[0:3, 0:3] = R_i_c
        J[0:3, 15:18] = np.identity(3)
        J[3:6, 0:3] = skew(R_w_i.T @ t_c_i)  #### to check why 
        # Alternatively if sign convention differs:
        # J[3:6, 0:3] = -R_w_i.T @ skew_symmetric(t_c_i)
        J[3:6, 12:15] = np.identity(3)
        J[3:6, 18:21] = R_w_i.T

        # Resize and augment the state covariance
        old_rows, old_cols = self.state_server.state_cov.shape
        self.state_server.state_cov = np.pad(
        ...
        # Fill in the augmented blocks
        self.state_server.state_cov[old_rows:old_rows+6, 0:old_cols] = J @ np.hstack((P11, P12))
        self.state_server.state_cov[0:old_cols, old_cols:old_cols+6] = self.state_server.state_cov[old_rows:old_rows+6, 0:old_cols].T
    ...
    def measurement_update(self, H, r):
        ...
        # Assume: H_thin, r_thin, and self.state_server.state_cov are defined
        P = self.state_server.state_cov

        # Measurement noise
        obs_noise = self.config.observation_noise 
        S = H_thin @ P @ H_thin.T + obs_noise * np.eye(H_thin.shape[0])

        # Solve for Kalman gain using symmetric LDLT-equivalent
        K_transpose = solve(S, H_thin @ P)#, assume_a='sym')
        K = K_transpose.T
        ...
        delta_vel_norm = np.linalg.norm(delta_x_imu[6:9])
        delta_pos_norm = np.linalg.norm(delta_x_imu[12:15])

        if delta_vel_norm > 0.5 or delta_pos_norm > 1.0:
            print(f"delta velocity: {delta_vel_norm:.3f}")
            print(f"delta position: {delta_pos_norm:.3f}")
            print("ROS_WARN: Update change is too large.")
            # return  # optionally skip the update

        # Apply orientation update (delta rotation)
        dq_imu = small_angle_quaternion(delta_x_imu[0:3])
        self.state_server.imu_state.orientation = quaternion_multiplication(
            dq_imu, self.state_server.imu_state.orientation
        )

        # Apply bias and state corrections
        ...
        # Apply small-angle correction to update IMU-to-camera rotation and translation.
        delta_theta = delta_x_imu[15:18]
        dq_extrinsic = small_angle_quaternion(delta_theta)
        dq_rotation = to_rotation(dq_extrinsic)
        self.state_server.imu_state.R_imu_cam0 = dq_rotation @ self.state_server.imu_state.R_imu_cam0
        ...
        for i, (cam_id, cam_state) in enumerate(self.state_server.cam_states.items()):
            delta_x_cam = delta_x[21 + i*6 : 21 + (i+1)*6]
