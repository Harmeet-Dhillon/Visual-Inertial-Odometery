...
class VIO(object):
    def __init__(self, config, img_queue, imu_queue, viewer=None):
        ...
        # Logging
        self.pose_log_data = []
        self.pose_log_initialized = False
    ...
    def process_feature(self):
        while True:
            ...
            if result is not None and self.viewer is not None:
                ...
                self.log_pose(result.timestamp, result.cam0_pose)
    
    # def log_pose(self, timestamp, pose):
    #     R = pose.R
    #     t = pose.t

    #     # Convert rotation matrix to quaternion
    #     q = to_quaternion(R)  # Your function returns [qx, qy, qz, qw] ordering

    #     timestamp = timestamp / 1e9

    #     # Prepare the data line: space-separated
    #     data = [timestamp, t[0], t[1], t[2], q[0], q[1], q[2], q[3]]

    #     # Save to txt
    #     mode = 'a' if self.pose_log_initialized else 'w'
    #     with open('stamped_traj_estimate.txt', mode) as f:
    #         if not self.pose_log_initialized:
    #             f.write("# time x y z qx qy qz qw\n")
    #             self.pose_log_initialized = True
    #         f.write(' '.join(f"{val:.18e}" for val in data) + '\n')
    ...
    def log_pose(self, timestamp, pose):
        ...
        data = [timestamp, t[0], t[1], t[2], q[0], q[1], q[2], q[3]]
        self.pose_log_data.append(data)

    def save_pose_log(self):
        with open('stamped_traj_estimate.txt', 'w') as f:
            f.write("# timestamp tx ty tz qx qy qz qw\n")
            for data in self.pose_log_data:
                f.write(' '.join(f"{val:.18f}" for val in data) + '\n')
